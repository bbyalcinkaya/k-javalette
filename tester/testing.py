...
def generate_input(source_file, N):
    for seed in range(N):
        child = subprocess.run(
                    ["krun", source_file, "-cRUN=1", "-cSEED=" + str(seed), "--output-file", "/dev/null"],
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE)
        infile = child.stderr.decode("utf-8")
        stdout_expected = child.stdout.decode("utf-8")
        yield (seed, infile, stdout_expected) 
...
def exec_test(exe, filename, is_good, linker, runner):
    ...
    for (seed, infile, stdout_expected) in generate_input(source_file, 100):
        ...
        if  stdout_actual != stdout_expected:
            return False, Struct(
